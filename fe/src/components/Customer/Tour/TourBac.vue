    ...
    <div class="container mt-3">
        <div class="row">
            <div class="text-center mb-3">
                <h4 class="text-uppercase fs-3 text-dark">Danh mục Tour Miền Bắc</h4>
                <p class="text-muted">Khám phá vẻ đẹp vùng núi phía Bắc Việt Nam</p>
            </div>
            ...
            <div v-else-if="listTour.length === 0" class="col-12 text-center py-5">
                ...
                <h5 class="text-muted">Chưa có tour nào thuộc Miền Bắc</h5>
            </div>
                            ...
                            <div class="position-relative">
                                ...
                                <div class="position-absolute top-0 end-0 m-2">
                                    <span class="badge bg-warning text-dark">
                                        <i class="fa-solid fa-tag"></i> Miền Bắc
                                    </span>
                                </div>
                            </div>
                        </div>
...
export default {
    data() {
        return {
            listTour: [],
            loading: true,
            // ID danh mục Miền Bắc - cần kiểm tra từ database
            mienBacCategoryId: 1,
        }
    },
    mounted() {
            ...
            axios.get("/customer/tour/get-data")
                .then(res => {
                    if (res.data && res.data.data) {
                        // Lọc tour Miền Bắc dựa trên id_danh_muc = 1
                        this.listTour = res.data.data.filter(tour => {
                            return tour.id_danh_muc == this.mienBacCategoryId && tour.so_cho_con > 0;
                        });
                        console.log('Tour Miền Bắc:', this.listTour);
                    }
                })
                .catch(err => {
                    console.error("Lỗi khi lấy tour Miền Bắc:", err);
                })
                .finally(() => {
                    this.loading = false;
                });
